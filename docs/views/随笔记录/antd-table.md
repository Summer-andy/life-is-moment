...
<Table
  onRow={() => {
    return {
      onMouseEnter: event => {
        const dom = event.target.querySelector('.cell');
        if(dom && !dom.className.includes('tooltip')) {
          return;
        }
        ...
        const rangeWidth = range.getBoundingClientRect().width;
        const padding = (parseInt(getStyle(dom.parentNode, 'paddingLeft'), 10) || 0) + (parseInt(getStyle(dom.parentNode, 'paddingRight'), 10) || 0);
          if ((rangeWidth + padding > dom.offsetWidth || dom.scrollWidth > dom.offsetWidth)) {
          console.log('object');
        }
...
本文是我在开发过程中的一些思考, 所涉及到的东西也只是冰山一山, 欢迎大家留下宝贵经验。