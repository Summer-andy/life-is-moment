...
感谢大家能够看到这里, 我相信看完文章的你, 一定收获了许多。可能你会觉得有点繁琐, 但是这些配置都是一劳永逸的。并且这个体系你可以随意切换UI库, 至于打包发布这些配置我们只需要让团队中的某位同学来负责搭建即可,其他同学的业务照常推进即可。之前用React开发PC端的, 照样也可以用React开发APP, 可以说几乎是"零"成本。并且我们只需要执行几个脚本就可以完成app的打包发布了。 由于ios的打包发布内容较多, 我打算另起文章, 大家有兴趣的可以期待一下 ~